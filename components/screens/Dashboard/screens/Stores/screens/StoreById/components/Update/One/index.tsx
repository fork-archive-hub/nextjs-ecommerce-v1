			...
			onSuccess: (result) => {
				trpcContext.setInfiniteQueryData(
					...
					(prevDataNullable) => {
						...
						if (result.newData.basicData) {
							targetedProduct = {
								...targetedProduct,
								status: result.newData.basicData.status,
								title: result.newData.basicData.title,
								price: result.newData.basicData.price,
								description: result.newData.basicData.description,
								countInStock: result.newData.basicData.countInStock,
								updatedAt: result.newData.basicData.updatedAt,
							};
						}

						if (result.newData.brandUpserted) {
						...
						// result.newData.
						if (result.newData.imagesCreated) {
							let images = targetedProduct.images || [];
							const removedImagesIds = removedRef.current.imagesIds;

							if (
								Array.isArray(removedImagesIds) &&
								removedImagesIds.length !== 0
							) {
							...
							targetedProduct = {
								...
								images: [
									...images,
									...result.newData.imagesCreated.map((item) => ({
										createdAt: new Date(),
										image: item,
									})),
								],
						...
						if (result.newData.categoriesUpserted) {
							let categories = targetedProduct.categories || [];
							const removedCategoriesNames = removedRef.current.categoriesNames;
							...
							targetedProduct = {
								...
								categories: [
									...categories,
									...result.newData.categoriesUpserted.map((item) => ({
										createdAt: new Date(),
